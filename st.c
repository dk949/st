...
int ttynew(char const *line, char *cmd, char const *out, char **args) {
    int m, s;
...
void csiprint(CSIEscape *csi) {
    char const *buf = csi->buf;
    size_t len = csi->len;
    char priv = csi->priv;
    int const *arg = csi->arg;
    int narg = csi->narg;
    char const *mode = csi->mode;
        ...
        case ']': /* OSC -- Operating System Command */
            switch (par) {
                ...
                case 1:
                    ...
                    return;
                case 2:
                    if (narg > 1) xsettitle(strescseq.args[1], 0);
                    return;
                case 52:
            ...
            break;
        case 'k': /* old title set compatibility */ xsettitle(strescseq.args[0], 0); return;
...
void tresize(int col, int row) {
    ...
    if (row < term.row || col < term.col) toggle_winmode(trt_kbdselect(XK_Escape, NULL, 0));
...
// TODO: have this take state, so it can be used with modifiers
int trt_kbdselect(KeySym ksym, char *buf, int len) {
    ...
    if (selectsearch_mode & 2) {
        ...
        if (ksym != XK_BackSpace) search(selectsearch_mode, &target[0], ptarget, sens, type, &cu);
    ...
    switch (ksym) {
        ...
        case XK_n:
        case XK_N:
            if (ptarget) search(selectsearch_mode, &target[0], ptarget, (ksym == XK_n) ? -1 : 1, type, &cu);
            break;
        case XK_BackSpace:
            term.c.x = 0;
            select_or_drawcursor(selectsearch_mode, type);
        ...
        case XK_exclam:
            ...
            break;
        case XK_asterisk:
        case XK_KP_Multiply: term.c.x = term.col >> 1;
        case XK_underscore:
            term.c.y = cu.y >> 1;
            select_or_drawcursor(selectsearch_mode, type);
            break;
        default:
            ...
            if (*xy == bound && ((sens < 0 && bound == 0) || (sens > 0 && bound > 0))) break;

            *xy += quant * sens;
            if (*xy < 0 || (bound > 0 && *xy > bound)) *xy = bound;
