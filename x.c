...
void xdrawglyphfontspecs(XftGlyphFontSpec const *specs, Glyph base, int len, int x, int y, int dmode) {
   ...
   if ((base.mode & ATTR_BOLD_FAINT) == ATTR_BOLD && BETWEEN(base.fg, 0, 7))
    ...
    if (base.mode & ATTR_SELECTED) {
        ...
        if (!ignoreselfg) fg = &dc.col[selectionfg];
    }
...
void usage(void) {
    die("usage: %s [-aiv] [-A alpha] [-c class] [-f font] [-g geometry] [-n name]\n"
        "          [-o file] [-T title] [-t title] [-w windowid] [[-e] command [args ...]]\n"
        "       %s [-aiv] [-A alpha] [-c class] [-f font] [-g geometry] [-n name]\n"
...
void keyboard_select(Arg const *dummy) {
